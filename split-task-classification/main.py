...
from utils.augmentation import get_train_transform, get_valid_transform
...
def main(args, logger, wandb):
    ...
    num_classes = 2 if args["TARGET"]=="gender" else 3
    ...
    if not test_mode:
        ...
        for fold_num, (train_index, valid_index) in enumerate(kfold.split(unique_path), start=1):
            ...
            train_dataset = MaskDataset(X_train, get_train_transform(args), args["TARGET"])
            train_loader = DataLoader(train_dataset, batch_size=args["BATCH_SIZE"], shuffle=True, num_workers=2)

            valid_dataset = MaskDataset(X_valid, get_valid_transform(args),args["TARGET"])
    ...
    if args["SAVE_LOGITS"]:
        ...
        for fold_num, (train_index, valid_index) in enumerate(kfold.split(unique_path), start=1):
            ...
            # Generate train data loader
            train_dataset = MaskDataset(X_train, get_valid_transform(args), args["TARGET"])
